...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const semitones = req.query.semitones
        ? parseInt(req.query.semitones)
        : -2;
      ...
      exec(
        `"${pythonPath}" "${pythonScript}" "${filePath}" ${semitones}`,
        async (error, stdout, stderr) => {
          ...
          if (error) {
            ...
            return res.status(500).json({ error: "Ошибка обработки Python" });
          }
